...
const LanguageSwitcher: React.FC = () => {
  ...
  return (
    <div className="relative">
      <button onClick={() => setIsOpen(!isOpen)} className="flex items-center text-dark-gray font-medium hover:text-primary transition-colors">
        {languages[language]}
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 bg-white rounded-md shadow-lg py-1 z-10">
    ...
    >
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {NAV_ITEMS.map((item) => (
              <a
                key={item.nameKey}
                href={item.href}
                className="text-dark-gray font-medium hover:text-primary transition-colors"
                aria-label={t(item.nameKey)}
              >
                {t(item.nameKey)}
              </a>
            ))}
             <a href="#login" className="text-dark-gray font-medium hover:text-primary transition-colors">{t('nav.login')}</a>
          </nav>
      ...
      >
        <div className="bg-white/95 backdrop-blur-sm shadow-lg px-4 pt-2 pb-4 space-y-2">
          ...
          <a href="#login" onClick={() => setIsMenuOpen(false)} className="block text-dark-gray font-medium py-2 hover:text-primary transition-colors">{t('nav.login')}</a>
